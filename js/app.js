...
async function boot() {
  ...
  try {
    ...
    if (engine === 'yolo') {
      _setAIStatus('ready', '✓ IA neuronal activa (YOLOv8)');
    } else {
      _setAIStatus('ready', '✓ Detector de color (sube model.onnx para IA neuronal)');
    }
  } catch(e) {
...
async function _startVideo() {
  ...
  // Callback de detección (función nombrada para reusar en pausa)
  const onDetections = dets => {
  ...
  if (pauseBtn) {
    pauseBtn.onclick = () => {
      ...
      if (ip)  ip.style.display  = paused ? 'none' : 'block';
      if (ipl) ipl.style.display = paused ? 'block' : 'none';
      if (paused) Detector.stopLoop();
      else Detector.startLoop(videoEl, onDetections);
    };
  }
}
...
function _evalFromVideo() {
  ...
  if (best.length > 0) {
    ...
    // Sin detecciones: evaluación manual sobre el último frame
    EvaluationUI.startSession([_manualDet(snap)], snap, 'video');
  }
}
