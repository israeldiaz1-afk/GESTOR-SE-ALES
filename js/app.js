...
function _startDetectionLoop(videoEl, canvasEl, countEl, evalBtn, countBadge, fpsEl) {
  Detector.startLoop(videoEl, canvasEl, detections => {
    // BUG2 FIX: anotar el tamaño fuente (vídeo nativo) en cada detección
    // para que detection-ui pueda escalar correctamente al overlay CSS
    const vW = videoEl.videoWidth  || canvasEl.width  || 640;
    const vH = videoEl.videoHeight || canvasEl.height || 480;
    const annotated = detections.map(d => ({ ...d, sourceW: vW, sourceH: vH }));

    // BUG3 FIX: en vídeo en tiempo real mostrar solo el frame actual (sin acumular)
    // Acumulamos para el botón "Evaluar señales" pero dibujamos solo lo del frame
    if (annotated.length > 0) {
      DetectionUI.accumulate(annotated);
    }
    // BUG4 FIX: siempre dibujar (incluso vacío para limpiar recuadros)
    DetectionUI.drawDetections(canvasEl, annotated);

    const total = DetectionUI.getAccumulated().length;
    countEl.textContent    = total;
    countBadge.textContent = total;
    evalBtn.disabled       = total === 0;
...
async function _handlePhotoFile(file) {
  ...
  try {
    ...
    // BUG5 FIX: detectar y anotar sourceW/H para escalar correctamente
    let detections = [];
    if (AppState.aiReady) {
      const rawDets = Detector.detectFrame(sized);
      // Anotar tamaño fuente = tamaño del canvas de análisis
      detections = rawDets.map(d => ({ ...d, sourceW: sized.width, sourceH: sized.height }));
